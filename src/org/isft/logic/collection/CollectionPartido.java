...
public class CollectionPartido extends AccessManager implements AccessInterface {
       public Vector select(HashMap param) throws Exception{
        Vector vec= new Vector();
        try{
        		String sql="SELECT * from partido where codigo is not null ";
            ...
            while(rst.next()){
                ...
                partido.setCodigo(rst.getString("codigo"));
                partido.setDescripcion(rst.getString("descripcion"));
                vec.add(partido);
            }             
        }catch(Exception exc){
