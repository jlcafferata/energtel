...
public class CollectionProvincia extends AccessManager implements AccessInterface {
       public Vector select(HashMap param) throws Exception{
        Vector vec= new Vector();
        try{
        		String sql="SELECT * from provincia where codigo is not null ";
            ...
            while(rst.next()){
                ...
                provincia.setCodigo(rst.getString("codigo"));
                provincia.setDescripcion(rst.getString("descripcion"));
                vec.add(provincia);
            }             
        }catch(Exception exc){
